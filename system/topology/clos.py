...
def main():
    ...
    os.system("sudo rm -f ../bmv2_model/flow_table/*")
    sw_num=nodes_list[0]*nodes_list[1]+(nodes_list[1]+nodes_list[2])*nodes_list[4]
    for i in range(sw_num):
        with open("../bmv2_model/flow_table/%d.txt" % (args.thrift_port+i), "w") as f:
            f.write("table_add int_table do_int => %d" %(i+1))
        os.system("sudo simple_switch_CLI --thrift-port %d <../bmv2_model/flow_table/%d.txt" % (args.thrift_port+i, args.thrift_port+i))
    ...
    #         for k in xrange(nodes_list[3]):
    #             h=net.get(topo.h_list[i][j][k])
    #             h.cmd("python ../packet/send/send_udp.py >/dev/null &")
